...
agregacion_existencias AS (
    ...
    JOIN `proyectofinalg2.proyectofinalG2.dim_bodega` b ON i.bodega_key = b.bodega_key
    GROUP BY 1, 2
)
SELECT
    p.nombre_producto,
...
JOIN `proyectofinalg2.proyectofinalG2.dim_producto` p ON e.producto_key = p.producto_key
LEFT JOIN agregacion_movimientos m ON e.producto_key = m.producto_key
